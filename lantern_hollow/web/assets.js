...
export function validatePack(m){
  ...
  if(typeof m.version!=='string'||m.version.length>128||typeof m.sha256!=='string'||!/^[a-f0-9]{64}$/.test(m.sha256))throw new Error('Invalid asset version or digest');
...
export async function loadSampleAssets(url=new URL('./sample-assets.json',import.meta.url),fetcher=fetch){
  ...
  try{
    ...
    const blob=await source.blob();if(blob.size>1048576)throw new Error('Atlas too large');const digest=await crypto.subtle.digest('SHA-256',await blob.arrayBuffer());const actual=[...new Uint8Array(digest)].map(n=>n.toString(16).padStart(2,'0')).join('');if(actual!==manifest.sha256)throw new Error('Atlas integrity check failed');const image=await createImageBitmap(blob);
