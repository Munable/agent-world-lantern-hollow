...
当前已验收服务器路径插值、气泡期限、脚本来源、公开角色关注、单调时钟及独立只读跨域查看器。0.3.0 另加入跟随、拖动、缩放和回位，见 [相机验收](CAMERA_ACCEPTANCE.md)。**公开关注不是控制权，公开跨域观察不是私有跨域读写；其余差距仍需逐项验证。**

v0.4 复现的 BubbleQueue 出队期限缺陷已在内核 0.13.1 修复，并由灯溪镇固定依赖使用；不另造世界端队列。相机现已使用一致正/逆坐标变换；鼠标拖动与触控缩放均有不误发移动指令的浏览器验收。
